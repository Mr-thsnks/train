        ...
        <td>
            <form class="form-inline">
                <select class="form-control" onChange="fun(this)">
                    <?php if(empty($iid)): ?><option value="sybj">- 请选择机构 -</option><?php endif; ?>
                    <?php if(!empty($iid)): ?><option value="<?php echo ($iid); ?>"><?php echo ($iid); ?></option><?php endif; ?>
                    <?php if(is_array($na)): $i = 0; $__LIST__ = $na;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["xqgl_name"]); ?>"><?php echo ($vo["xqgl_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
                <a href="<?php echo U('Index/xqglmain');?>" class="btn btn-success">还原</a>
            </form>
        </td>
        <td width="100px;">
...
<table class="table table-bordered center-block" style="width:97.5%; margin-top:0px;">
    ...
    <?php if($results): if(is_array($results)): foreach($results as $key=>$val): ?><tr align="center">
                <td><?php echo ($val["organizat"]); ?></td>
                <td><?php echo ($val["principal"]); ?></td>
                <td><?php echo ($val["phone"]); ?></td>
                <td><?php echo ($val["addr"]); ?></td>
                <td><?php echo ($val["inside_call"]); ?></td>
                <td><?php echo ($val["sort"]); ?></td>
                <td><?php echo ($val["add_time"]); ?></td>
                <td width="160px;">
                    <a href="<?php echo U('/branch/update', array('table'=>I('get.table'), 'id'=>$val['id']));?>" class="btn btn-warning">编辑</a>
                    <a href="<?php echo U('/branch/del', array('table'=>I('get.table'), 'id'=>$val['id']));?>" class="btn btn-danger" >删除</a>
                </td>
            ...
            <div class="modal-body">
                <form action="<?php echo U('');?>" method="post" class="validatable">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" placeholder="创建时间" name="xqgl_guoqishijian" value="<?php echo ($shijian); ?>" readonly>
                    </div>
                     -->
