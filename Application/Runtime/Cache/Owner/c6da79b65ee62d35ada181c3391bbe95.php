...
<div class="ulb">
    ...
    <ul>
        <li class="btn btn-primary" role="button">
            <a href="<?php echo U('/marketing', array('bid'=>I('get.bid')));?>">市场<span class="glyphicon glyphicon-education" aria-hidden="true"></span></a>
        </li>
    ...
    <tr>
        <td>
            ...
            <input type="text" name="jiaoshi" placeholder="请输入手机号码/姓名"/>
                ...
                <?php if(!empty($jiaoshi)): ?><a href="?m=Admin&c=login&a=qxzhmain&id=<?php echo (session('userid')); ?>&xqid=<?php echo (session('xiaoquid')); ?>" class="btn btn-info">还原</a><?php endif; ?>
            </form>
        </td>
        <td width="110px">
            ...
            <div class="modal fade" id="create-user" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <form action="<?php echo U('');?>" method="post" class="validatable">
                            ...
                            <div class="modal-body">
                                <table class="table table-bordered">
                                        ...
                                        <td>
                                            <div class="col-md-10">
                                                <select name="branch_id" class="form-control" data-validation-engine="validate[required]">
                                                    ...
                                                    <?php if($branch): if(is_array($branch)): foreach($branch as $key=>$val): ?><option value="<?php echo ($val["id"]); ?>"><?php echo ($val["organizat"]); ?></option><?php endforeach; endif; endif; ?>
                                                </select>
                                            </div>
                                        </td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td>
                                            <?php $auth = getAuth(); ?>
                                            <?php if(is_array($auth)): foreach($auth as $key=>$val): ?><!-- <label for=""><?php echo ($val["title"]); ?><input type="radio" name="auth[]" value="<?php echo ($val["id"]); ?>"></label> -->
                                                <label class="checkbox-inline">
                                                    <input type="radio" name="auth" value="<?php echo ($val["id"]); ?>"> <?php echo ($val["title"]); ?>
...
<table class="table table-bordered center-block" style="width:97.5%; margin-top:0px;">
    ...
    <?php if($results): if(is_array($results)): foreach($results as $key=>$val): ?><tr>
                ...
                <td align="center"><?php echo (findBranch($val["branch_id"], 'organizat')); ?></td>
                <td align="center"><?php echo (findAuth($val["id"])); ?></td>
                <td style="width:180px; text-align:center">
                    <a href="<?php echo U('/branch/update', array('table'=>I('get.table'), 'id'=>$val['id']));?>" class="btn btn-success btn-xs">编辑</a>
                    <a href="<?php echo U('/branch/del', array('table'=>I('table'), 'id'=>$val['id']));?>" class="btn btn-danger btn-xs">删除</a>
                </td>
